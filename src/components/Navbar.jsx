...
const Navbar = () => {
  ...
  return (
    <nav
      className={`${styles.paddingX} w-full flex border items-center py-2 sticky top-0 z-20 bg-[#D9E8F7]`}
    >
        ...
        <ul className="list-non hidden sm:flex flex-row gap-10">
          {navLinks.map((link) => (
            <li
              key={link.id}
              className={`${
                active === link.title ? "text-[brown]" : "text-primary"
            ...
            <ul className="list-non flex justify-end items-start flex-col gap-4">
              {navLinks.map((link) => (
                <li
                  key={link.id}
                  className={`${
                    active === link.title ? "text-[brown]" : "text-[#D9E8F7]"
